...
size_t find_shortest_path(size_t** g, bit_set_t* bit_set) {
    ...
    while (previous_gen) {
        ...
        if (ret > current_member->value + g[current_member->last_j][next_stop_index] + g[next_stop_index][0]) {
            ret = current_member->value + g[current_member->last_j][next_stop_index] + g[next_stop_index][0];
        }
